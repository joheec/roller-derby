...
class SSRD {
    ...
    mine (req, res, next) {
        ...
        // Create a new transaction with from 0 (this node) to our node (NODE_NAME) of 1 SSRD 
    ...
    newTransaction (req, res, next) {
        ...
        const index = this.blockchain.newTransaction(trans['verifier'], trans['derbyGirl'], trans['item'])
        const responseValue = {
    ...
    checkLogin (req, res, next) {
        console.log('In backend checkLogin 1');
        const errors = validationResult(req)
        if (!errors.isEmpty()) {
            return res.status(422).json({ errors: errors.mapped() });
        }

        console.log('In backend checkLogin 2');
        console.log(req.body);
    }
}

module.exports = new SSRD()
